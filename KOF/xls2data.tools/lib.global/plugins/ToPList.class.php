...
class ToPList 
{
	...
	private function xmlWriteValue(\XMLWriter $x, $v,$k='') 
	{
		//echo '$v:';print_r($v);echo '<br />';
		if(is_numeric($v) && strlen($v) < 20 )
		{
			$v = $v+0;
		...
		{
			trigger_error("Unsupported data type in plist ({$k}:{$v}) ", E_USER_WARNING);
			//exit();
			$x->writeElement('string', $v);
		}
	}
	...
	private function textWriteValue(&$text, &$v, $indentLevel = 0) 
	{
		...
		{
			trigger_error("Unsupported data type in plist  ({$text}:{$v})", E_USER_WARNING);
			$this->textWriteString($text, $v); 
		}
	}
