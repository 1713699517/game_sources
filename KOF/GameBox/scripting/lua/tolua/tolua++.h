...
#ifndef CCtolua_release
//#define CCtolua_release(EXP) delete EXP
#define CCtolua_release(EXP) \
    CCObject *___xpObj = dynamic_cast<CCObject *>(EXP); \
    if( ___xpObj != NULL && ___xpObj->retainCount() > 0 ){ ___xpObj->release(); }else{ delete EXP; }
