...
C360_Android_Recharge::C360_Android_Recharge()
:m_phightImg(NULL)
,m_pselectImg(NULL)
...
bool C360_Android_Recharge::init()
{
    if( !CCNode::init() )
        return false;
...
void C360_Android_Recharge::initParameter()
{
    //可选金额
...
void C360_Android_Recharge::initView()
{
    ...
    mainContainer->setControlName("this C360_Android_Recharge mainContainer 111");
    ...
    m_pCloseButton->setControlName("this C360_Android_Recharge m_pCloseButton");
    ...
    m_pCloseButton->addEventListener("TouchBegan", this, eventhandler_selector(C360_Android_Recharge::closeButtononBeganTouch));
    mainContainer->addChild(m_pCloseButton);
    
    //标题:充值中心
    CSprite *pTitle = CSprite::createWithSpriteFrameName("recharge_word_czzx.png");
    pTitle->setControlName("this C360_Android_Recharge pTitle 65");
    ...
    pGongGao->setControlName("this C360_Android_Recharge pGongGao 65");
    ...
    pGongGaoTitle->setControlName("this C360_Android_Recharge pGongGaoTitle 65");
    pGongGaoTitle->setPosition(ccp( pGongGao->getPreferredSize().width/2.0f - 7.0f, 515 ));
    mainContainer->addChild(pGongGaoTitle, 11);
    //公告内容
    string rechargeInfo = ptola::configuration::CLanguageManager::sharedLanguageManager()->getString(("RechargeInfo"));//RechargeInfo
    CCLabelTTF *pRechargeInfo = CCLabelTTF::create( rechargeInfo.c_str(), "Arial", 20); //rechargeInfo.c_str()
    ...
    pMainBg->setControlName("this C360_Android_Recharge pMainBg 65");
    ...
    pRechaeTitle1->setControlName("this C360_Android_Recharge pRechaeTitle1 65");
    pRechaeTitle1->setPosition(ccp( pos_X + pRechaeTitle1->getPreferredSize().width/2.0f + 120, 532 ));
    ...
    pLineImg->setControlName("this C360_Android_Recharge pLineImg 65");
    ...
    pRechaeTitle2->setControlName("this C360_Android_Recharge pRechaeTitle2 65");
...
//    }
    ...
    m_pRechargeBtn->setControlName("this C360_Android_Recharge m_pRechargeBtn");
    CCSize rechargeBtnSize = m_pRechargeBtn->getPreferredSize();
    m_pRechargeBtn->setPosition(ccp( pos_X+490,108));
    m_pRechargeBtn->addEventListener("TouchBegan", this, eventhandler_selector(C360_Android_Recharge::rechargeButtononBeganTouch));
    ...
    CCScale9Sprite *_editBg = CCScale9Sprite::createWithSpriteFrameName("general_second_underframe.png");
    m_pEditBox = CEditBox::create( editBoxSize, _editBg, 6, "", kEditBoxInputFlagSensitive);
    m_pEditBox->setEditBoxInputMode( kEditBoxInputModePhoneNumber);
    m_pEditBox->setPosition(ccp( visibleSize.width/2, visibleSize.height*0.65f ));
    addChild(m_pEditBox);
    
    m_pEditBox->setTextString("10");
     */
    
    
}

CButton *C360_Android_Recharge::createItem( int money )
{
    char str_money[100];
    ...
    button->setControlName("this C360_Android_Recharge button 250");
    button->setPreferredSize( pCellSize );
    button->setTag(money);
    button->addEventListener("TouchBegan", this, eventhandler_selector(C360_Android_Recharge::selectMoneyBeganTouch));
    
    //背景圈圈
    CSprite *rideoBg = CSprite::createWithSpriteFrameName("recharge_underframe_nomal.png");
    rideoBg->setControlName("this C360_Android_Recharge createItem rideo 194");
    ...
    rideo->setControlName("this C360_Android_Recharge createItem rideo 194");
...
void C360_Android_Recharge::setHightLight( CButton *obj )
{
    //设置选中项的背景高亮效果
    if( m_phightImg != NULL )
    {
...
void C360_Android_Recharge::selectMoneyBeganTouch(cocos2d::CCObject *pSender, ptola::event::CEvent *pEvent)
{
...
void C360_Android_Recharge::closeButtononBeganTouch(cocos2d::CCObject *pSender, ptola::event::CEvent *pEvent)
{
...
void C360_Android_Recharge::rechargeButtononBeganTouch(cocos2d::CCObject *pSender, ptola::event::CEvent *pEvent)
{
    CCLOG("Recharge rechargeButtononBeganTouch-->",pEvent);
    
//    string _money_str = m_pEditBox->getTextString();   //atoi(_money_str.c_str());
    int _money = pMoney;  //选择的金额 单位：元
    ...
    char cSid[256];
    strcpy( cSid, serverid);
    
    char cRoleId[256];
    strcpy( cRoleId, roleid);

    CCLOG("所需充值金额 %s",  serverid);
    CCLOG("roleid--> %s", roleid);
    JniMethodInfo _info;//定义Jni函数信息结构体 
    if ( JniHelper::getStaticMethodInfo(_info,"com/haowan123/kof/qh/GameBox", "recharge", "(ILjava/lang/String;Ljava/lang/String;)V") )
    {
        jint money      = _money;
        jstring _serverid = _info.env->NewStringUTF( cSid );
        jstring _roleid = _info.env->NewStringUTF( cRoleId );
        
        //调用此函数
        _info.env->CallStaticVoidMethod(_info.classID, _info.methodID, money,  _serverid, _roleid);
        
    }
    else
    {
        CCLOG("C360_Android_Recharge::getNetworkStatus method missed!");
    }
