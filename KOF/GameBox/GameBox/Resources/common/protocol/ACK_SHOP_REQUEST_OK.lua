...
ACK_SHOP_REQUEST_OK = class(CAcknowledgementMessage,function(self)
	self.MsgID = Protocol.ACK_SHOP_REQUEST_OK
	self:init()
end)

function ACK_SHOP_REQUEST_OK.deserialize(self, reader)
	self.type     = reader:readInt16Unsigned()
    self.type_bb  = reader:readInt16Unsigned()
    self.count    = reader:readInt16Unsigned()
    
    print("ACK_RENOWN_REQUEST_OK.deserialize---->>>>>>>",self.type,self.type_bb,self.count)
    ...
    while icount <= self.count do
        ...
        self.goods_msg_no[icount].total_remaider_Num = reader: readInt16Unsigned()
        self.goods_msg_no[icount].goods_num          = reader: readInt16Unsigned() --兑换物品数量
        if tonumber(self.goods_msg_no[icount].s_price) == 0 then
            self.goods_msg_no[icount].s_price = self.goods_msg_no[icount].goods_num
        end
        print("0099900",self.goods_msg_no[icount].total_remaider_Num)
        print("self.goods_msg_no[icount].idx",self.goods_msg_no[icount].idx)
        icount = icount + 1
    end
    
    
	self.end_time = reader:readInt32Unsigned() 
 
end
-- {店铺类型}
function ACK_SHOP_REQUEST_OK.getType(self)
	return self.type
end

-- {子店铺类型}
function ACK_SHOP_REQUEST_OK.getTypeBb(self)
	return self.type_bb
end

-- {物品数量}
function ACK_SHOP_REQUEST_OK.getCount(self)
	return self.count
end

-- {信息块34501}
function ACK_SHOP_REQUEST_OK.getMsg(self)
	return self.goods_msg_no
end

-- {结束时间}
function ACK_SHOP_REQUEST_OK.getEndTime(self)
