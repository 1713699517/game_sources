...
require "mediator/BottomHalfLayerMediator"

CBottomHalfLayer = class(view,function (self)
                          end)

CBottomHalfLayer.TAG_HappyBtn = 500

function CBottomHalfLayer.scene(self)
...
function CBottomHalfLayer.layer(self)
    local winSize = CCDirector:sharedDirector():getVisibleSize()
    self.Scenelayer    = CContainer :create()
...
function CBottomHalfLayer.loadResources(self)
    _G.Config:load("config/goods.xml")
end

function CBottomHalfLayer.layout(self, winSize)  --适配布局
    local IpadSize = 854
...
function CBottomHalfLayer.init(self, _winSize, _layer)
...
function CBottomHalfLayer.initParameter(self)
    self : registerMediator()   --mediator注册
    -- self : REQ_WRESTLE_FINAL_REQUEST () --决赛入口
end

function CBottomHalfLayer.initView(self,_winSize,_layer)
    ...
    for i=1,8 do
        ...
        self.LeftDividingSprite[i] : registerControlScriptHandler(CallBack,"this CBottomHalfLayer RightDividingSprite 110")
    ...
    for i=1,8 do
        ...
        self.RightDividingSprite[i] : registerControlScriptHandler(CallBack,"this CBottomHalfLayer RightDividingSprite 110")
    ...
    self.UpperHalfSprite       = CSprite : createWithSpriteFrameName("combat_word_xbq.png")
    ...
    self.HappyBtn : registerControlScriptHandler(CallBack,"this CBottomHalfLayer HappyBtn 132")
    self.HappyBtn : setTag(CBottomHalfLayer.TAG_HappyBtn)
...
function CBottomHalfLayer.CallBack(self,eventType,obj,x,y)  --关闭页面按钮回调
   if eventType == "TouchBegan" then
        return obj:containsPoint( obj:convertToNodeSpaceAR(ccp(x,y)) )
    elseif eventType == "TouchEnded" then
        local  TAG_value  = obj : getTag()
        if     TAG_value == CBottomHalfLayer.TAG_HappyBtn then
            print("欢乐小竞猜")
...
function CBottomHalfLayer.changeLeftBoxSprite(self,TAG_value)
...
function CBottomHalfLayer.changeRightBoxSprite(self,TAG_value)
    self : changeSprite() --消除之前痕迹
...
function CBottomHalfLayer.changeSprite(self)
...
function CBottomHalfLayer.pushData(self,Count,Msg) --sever methond
    print("CBottomHalfLayer.pushData",Count)
    self.Msg              = Msg
...
function CBottomHalfLayer.initFourPlayerView( self ,firstdata,nowdata) --显示第四轮结果
...
function CBottomHalfLayer.initThirdPlayerView( self ,firstdata,nowdata) --显示第三轮结果
    if  firstdata ~= nil then
        local Count    = firstdata.count 
        if Count > 0 then
            for i = 1,Count do
...
function CBottomHalfLayer.initSecondPlayerView( self ,firstdata,nowdata) --显示第二轮结果
    if  firstdata ~= nil then
        local Count    = firstdata.count 
        if Count > 0 then
            for i = 1,Count do
...
function CBottomHalfLayer.MsgDataGrouping( self, Msg,RoundNo)
    local data = {}
    local count = 0 
...
function CBottomHalfLayer.initDividingLabelData(self,Count,data) --初始化第一轮数据
...
--mediator 注册
function CBottomHalfLayer.registerMediator(self)
    print("CBottomHalfLayer.mediatorRegister 75")
    _G.g_BottomHalfLayerMediator = CBottomHalfLayerMediator (self)
    controller :registerMediator(  _G.g_BottomHalfLayerMediator )
end
--协议发送
-- function CBottomHalfLayer.REQ_WRESTLE_FINAL_REQUEST(self) -- [54850]请求下半区数据 -- 格斗之王  决赛入口
...
function CBottomHalfLayer.NetWorkReturn_WRESTLE_STATE(self,State) --sever methond
    print("CBottomHalfLayer.NetWorkReturn_WRESTLE_STATE",State)
    local msg = ""
    if State ~= nil then
...
function CBottomHalfLayer.NetWorkReturn_WRESTLE_TIME(self,Type,Value) --sever methond
    print("CBottomHalfLayer.NetWorkReturn_WRESTLE_TIME",Type,Value)
...
----倒计时
function CBottomHalfLayer.registerEnterFrameCallBack(self)
    print( "CBottomHalfLayer.registerEnterFrameCallBack")
...
function CBottomHalfLayer.updataReceiveAwardsTime( self, _duration)
...
function CBottomHalfLayer.setReceiveAwardsTime(self, _time)
...
function CBottomHalfLayer.turnTime( self, _time)
...
--{时间转字符串}
function CBottomHalfLayer.toTimeString( self, _num )
...
function CBottomHalfLayer.NetWorkReturn_WRESTLE_FINAL_REP( self,Turn,Count,Msg)
    print("CBottomHalfLayer收到新协议发过来的数据",Turn,Count,Msg)
...
function CBottomHalfLayer.initNewDividingLabelData(self,Turn,data) 
    if data ~= nil then
