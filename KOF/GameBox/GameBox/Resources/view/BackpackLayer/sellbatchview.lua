...
CSellBatchView = class(view, function( self)
end)

--批量出售最大数量
CSellBatchView.MAX_GOODS_NUM = 9

--加载资源
function CSellBatchView.loadResource( self)
    CCSpriteFrameCache :sharedSpriteFrameCache() :addSpriteFramesWithFile("ImageBackpack/backpackUI.plist") 
end

--释放资源
function CSellBatchView.onLoadResource( self)
    
end

--释放成员
function CSellBatchView.realeaseParams( self)
    self.m_goods = {}
...
--布局成员
function CSellBatchView.layout( self, winSize)    
...
--初始化成员
function CSellBatchView.initview( self, layer)
...
--初始化批量容器
function CSellBatchView.initScrollView(self, layer)
    
    local sellbatchview = CContainer :create()
    sellbatchview : setControlName( "this is CSellBatchView sellbatchview 57")
    
    local bgImg = CSprite :createWithSpriteFrameName("iconinfo.png")
    bgImg : setControlName( "this CSellBatchView bgImg 60 ")
    ...
    sellButton : setControlName( "this CSellBatchView sellButton 81 " )
    sellButton: setFontSize(30)
    sellButton: registerControlScriptHandler( sellButtonCallBack, "this CSellBatchView sellButton 84")
    ...
    for k =1 ,1 do
        ...
        pagecontainer : setControlName( "this is CSellBatchView pagecontainer 98")
        ...
        smallPoint : setControlName( "this CSellBatchView smallPoint 113 " )
        smallPoint.view = self
        smallPoint :registerControlScriptHandler(self.clickedSmollpointCallBack, "this CSellBatchView smallPoint 116")
        ...
        for i =1 ,9 do
            ...
            self.m_goodBtn[k][i] : setControlName( "this CSellBatchView self.m_goodBtn[k][i] 122 "..tostring(k).."  "..tostring(i))
           ...
           --m_bg :setPosition(ccp(0,0))
            ...
            self.m_goodBtn[k][i] :registerControlScriptHandler(clickedCellCallBack, "this CSellBatchView self.m_goodBtn[k][i] 157")
...
--主界面初始化
function CSellBatchView.init(self, winSize, layer)
       
    CSellBatchView.loadResource(self)
...
function CSellBatchView.getSellAllPrice( self)
    local i = 1
    local allprice = 0
...
function CSellBatchView.getSellNumber( self)
    local i = 0
    local sellnumber = 0
    while i < table.maxn(self.m_goods) do
...
--增加待售物品
function CSellBatchView.addGoodToSell(self, good)
    print("CSellBatchView.addGoodToSell")
    if self.getSellNumber( self) == CSellBatchView.MAX_GOODS_NUM then
        --提示达到最大出售数量
        --add:
        print("达到最大出售数量",CSellBatchView.MAX_GOODS_NUM)
        return
    end
...
--减少待售物品
function CSellBatchView.subGoodToSell(self, good)
    print("CSellBatchView.subGoodToSell")
    if good == nil then
        --没有选中物品
        print("NO GOOD")
        return
...
--批量出售中是否有good
function CSellBatchView.isContain(self, good)
    print("CSellBatchView.isContain")
    if good == nil then
        return
    end
    local i = 1
...
--比较good1和good2
function CSellBatchView.compareTwoGood(self, good1, good2)
    print("CSellBatchView.compareTwoGood")
...
--设置容器大小
function CSellBatchView.setContainerSize(self, counts)
    
end

--显示批量出售中的物品
function CSellBatchView.showSellGood(self, goods)
    print("CSellBatchView.showSellGood")
    --批量出售窗口中物品信息初始化
    --add:
        for i=1,1 do
            for j=1,9 do
        ...
        for k,v in pairs(goods) do
            ...
            self.m_goodBtn[i][j].icon : setControlName( "this CSellBatchView self.m_goodBtn[i][j].icon 337 "..tostring(i).." "..tostring(j))
...
--出售成功刷新界面
function CSellBatchView.removeGoodsByIndex(self, index)
    print("CSellBatchView.removeGoodsByIndex")
    local i = 1
...
--卖掉所有goods
function CSellBatchView.sellAllGoods(self)
    if self:getSellNumber() <= 0 then
        --提示没有东西出售
        --add:
...
function CSellBatchView.layer(self)
    local winSize = CCDirector:sharedDirector():getVisibleSize()
    self.m_layer = CContainer :create()
    self.m_layer : setControlName("this is CSellBatchView self.m_layer 411")
    self.m_layer :setPosition(winSize.width/2,winSize.height/2)
    self: init(winSize, self.m_layer)
    return self.m_layer
end

function CSellBatchView.scene(self)
    local winSize = CCDirector :sharedDirector() :getVisibleSize()
