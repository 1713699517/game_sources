...
pid_send(Pid, Msg) when is_pid(Pid) ->  
	Pid ! Msg , 
	?true;
pid_send(RegName, Msg) when is_atom(RegName) ->
...
%% 给在线玩家发信息,发送成功后在work_pro中区进行匹配处理
%% 返回：?true | ?false
pid_send(Mpid, Mod, Fun, Arg) when is_pid(Mpid) ->
	case pid_send(Mpid,{?exec, Mod, Fun, Arg}) of
		?true  -> 
			?true;
		?false -> 
			?false
	end;
pid_send(RegName, Mod, Fun, Arg) when is_atom(RegName) ->
	case whereis(RegName) of
		Pid when is_pid(Pid)  -> 
			pid_send(Pid, Mod, Fun, Arg);
		_undefined 			  -> 
			?false
	end;
pid_send(Uid, Mod, Fun, Arg) when is_number(Uid) ->
	case role_api:mpid(Uid) of
		MPid when is_pid(MPid)  -> 
			pid_send(MPid, Mod, Fun, Arg);
		_null -> 
			?false
	end;
pid_send(_null, _Mod, _Fun, _Arg) -> 
	?false.
...
%% 返回 true  or false
rand_percent(Percent) when Percent >= ?CONST_PERCENT ->  ?true;
rand_percent(Percent) -> rand_odds(Percent, ?CONST_PERCENT).

%% 机率
%% Numerator   分子
%% Denominator 分母
rand_odds(Denominator,Denominator) -> ?true;
rand_odds(Denominator,Numerator) when Denominator > Numerator ->
	rand_odds(Numerator,Denominator);
rand_odds(Numerator,Denominator)->
	Odds = ?MODULE:uniform(Denominator),
	if Odds =< Numerator -> ?true;
	   ?true -> ?false
	end.
...
%% 从列表中随机取出一个数据(配置概率)
%% odds_api:odds_list(List, Denominator).
%% odds_api:odds_list([{a,20},{b,45},{c,85},{d,90},{e,100}],100).
odds_list(OddsList, OddsListSun) ->
	Odds = ?MODULE:uniform(OddsListSun),
	case odds_list2(Odds, OddsList) of
		?null ->
			?MSG_ERROR("Odds:~p OddsListSun:~p OddsList:~p ",[Odds,OddsListSun,OddsList]), 
			?null;
		Value ->
			Value
	end.
...
odds_list_count([],      _Denominator,_List,_Count,_Num, Acc)->
	Acc;
odds_list_count(_,      _Denominator,_List,0,_Num, Acc)->Acc;
...
odds_list_init(List) ->
	Fun = fun({_Value, 0}, Acc) ->
			 ...
			 ({Value, Odds}, {Acc, OddsAcc}) ->
				  ...
				  {[{Value, NewOdds}|Acc], Odds + OddsAcc}
		  end,
...
lists_group([H|T],Acc) ->
	...
	NewT	= lists:foldl(Fun,[],T),
	Count	= SumLength - length(NewT),
	lists_group(NewT,[{H,Count} | Acc]);
lists_group([],Acc) ->
	Acc.
...
%% 1323397624
seconds()-> 
	{MegaSecs2, Secs2, _MicroSecs2} = ?TIME_NOW(),
...
is_same_date(Seconds1, Seconds2) ->
    {{Year1, Month1, Day1}, _Time1} = seconds2localtime(Seconds1),
    {{Year2, Month2, Day2}, _Time2} = seconds2localtime(Seconds2),
    if 
...
ip(Socket) ->	
	case inet:peername(Socket) of
		{?ok, {IP,_Port}} ->
			{Ip0,Ip1,Ip2,Ip3}  = IP,
			integer_to_list(Ip0)++"."++integer_to_list(Ip1)++"."++integer_to_list(Ip2)++"."++integer_to_list(Ip3);
		_-> 
			"0.0.0.0"
	end.

%%io:format("~w",[U]),
%% integer_to_list(192)++"."++integer_to_list(168)++"."++integer_to_list(1)++"."++integer_to_list(122).
